...
def get_client(acc_id, session_id, proxy=None, csrf_token=None):
    ...
    # CSRF token assignment removed (instagrapi handles it)
    ig_clients[acc_id] = cl
    return cl
...
def bot_worker(acc_id, acc, stop_event):
    ...
    csrf_token = acc.get("csrf_token", "").strip() or None  # stored but not used
    ...
    while not stop_event.is_set():
        ...
        # Periodic NC after every N messages
            ...
            try:
                ...
                if status_code == 403 or "user_has_logged_out" in err_str or "login_required" in err_str:
                    ...
                    try:
                        ...
                        log(acc_id, "✅ Re-login successful — resuming")
                        bot_status[acc_id]["last_action"] = "Re-login done ✓"
                    except Exception as re_err:
                        log(acc_id, f"❌ Re-login failed: {re_err} — stopping bot")
                        bot_status[acc_id]["running"] = False
                        bot_status[acc_id]["last_action"] = f"Re-login failed: {re_err}"
                        return
                else:
